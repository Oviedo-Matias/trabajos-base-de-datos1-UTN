...
BEGIN
    ...
    SELECT numero_movimiento INTO var_num_movimiento FROM movimientos WHERE numero_cuenta = cuenta LIMIT 1;
...
BEGIN
	...
	IF NEW.tipo = 'CREDITO' THEN
		UPDATE cuentas SET cuentas.saldo = cuentas.saldo + NEW.importe WHERE cuentas.numero_cuenta = NEW.numero_cuenta;
        SELECT saldo INTO var_saldo_actual FROM cuentas WHERE numero_cuenta = NEW.numero_cuenta;
        INSERT INTO historial_movimientos (numero_cuenta, numero_movimiento, saldo_anterior, saldo_actual) VALUES 
			(NEW.numero_cuenta, NEW.numero_movimiento, var_saldo_anterior, var_saldo);
	ELSE
		UPDATE cuentas SET cuentas.saldo = cuentas.saldo - NEW.importe WHERE cuentas.numero_cuenta = NEW.numero_cuenta;
        SELECT saldo INTO var_saldo_actual FROM cuentas WHERE numero_cuenta = NEW.numero_cuenta;
        INSERT INTO historial_movimientos (numero_cuenta, numero_movimiento, saldo_anterior, saldo_actual) VALUES 
...
-- Se elimina el trigger para evitar consultas repetidas
DROP TRIGGER IF EXISTS ActualizarSaldo;
